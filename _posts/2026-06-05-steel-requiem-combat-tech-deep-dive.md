...
이 글은 Unity에서 적을 찾고, 칼의 궤적과 충돌을 계산하고, 맞은 순간 화면에 피드백을 주는 흐름을 한 번에 따라간다. Unity의 `MonoBehaviour`와 2D 물리 API를 사용해 본 독자를 기준으로 하며, 프로젝트에서만 쓰는 클래스 이름보다 각 코드가 해결하는 문제를 먼저 설명한다.

먼저 자주 나오는 용어를 짚고 간다.

- **DI(Dependency Injection, 의존성 주입)**: 객체가 필요한 다른 객체를 내부에서 직접 만들지 않고 밖에서 전달받는 방식이다.
- **FSM(Finite State Machine, 유한 상태 머신)**: 대기·추적·공격처럼 가능한 상태와 전환 규칙을 나눠 행동을 관리한다.
- **VFX(Visual Effects)**: 칼 궤적, 충격파, 혈흔처럼 타격을 눈으로 전달하는 효과다.
- **FOV(Field of View)**: 적이 볼 수 있는 각도 범위다.

코드를 다시 들여다보니 프로젝트를 관통하는 설계 규칙이 두 개 있었다.
