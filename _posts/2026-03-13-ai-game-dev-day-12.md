...
## 패링만 있던 프로토타입에 공격과 반격을 연결하기

11일차 프로토타입은 적의 공격을 박자에 맞춰 막는 데서 끝났다. 그것만으로는 플레이어가 먼저 공격하고, 성공한 패링을 반격으로 이어 가는 전투 흐름을 시험할 수 없었다. 12일차 목표는 **공격 → 적의 반응 → 패링 → 반격**이 다시 공격으로 돌아오는 한 사이클을 만드는 것이었다.

**HitStop**은 타격 순간 게임 화면을 아주 짧게 멈춰 충격을 강조하는 연출이다. **반격 윈도우**는 패링 성공 후 반격을 입력할 수 있는 제한 시간이다. 이 글에서는 시간을 초가 아니라 비트 수로 세어 음악 속도가 바뀌어도 같은 박자 감각을 유지하는 과정을 설명한다.
...
12일차에는 비트에 맞춘 공격, 4비트 동안의 반격 기회, 반격 적중 시 HitStop까지 연결했다. 여기서 "완성"은 이 전투 사이클이 프로토타입에서 실행된다는 뜻이며, 밸런스와 최종 애니메이션까지 끝났다는 뜻은 아니다.
