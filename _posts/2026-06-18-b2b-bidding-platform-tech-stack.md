...
async refresh(refreshToken: string, ip?, ua?): Promise<TokenPair> {
  ...
  const tokens = await this.signTokens(session.userId, /* role */, ip, ua);
...
```text
인터넷 → CloudFront(웹 정적자산, S3 origin) 
       → ALB(HTTPS) → ECS Fargate(NestJS API)
                          ├─ RDS PostgreSQL 16 (prod: Multi-AZ)
                          ├─ ElastiCache Redis (BullMQ·캐시)
                          └─ S3 (계약 PDF·서명·사업자증)
```
