---
title: "택배 입찰 플랫폼 개발기 — React·NestJS·AWS를 선택한 기준"
description: "화주와 택배 대리점이 견적을 비교하고 계약하는 플랫폼을 만들며, 실시간 방식·모바일 앱·인증·클라우드 구성을 어떤 기준으로 선택했는지 정리한다."
date: 2026-06-18
categories: [Project]
...
이 플랫폼에는 두 사용자가 있다. **화주**는 보낼 물량과 조건을 등록하고, **택배 대리점**은 처리 가능한 가격을 제안한다. 여러 제안을 같은 화면에서 비교하고 선택한 뒤 계약 기록을 남기는 것이 핵심 흐름이다. 여기서 B2B는 개인 소비자가 아니라 사업자끼리 거래한다는 뜻이다.
