---
title: "AI로 게임 개발하기 5일차 — 36개 기능 목록을 구현 단위로 바꾸기"
description: "확정한 게임 기획을 의존성 순서로 나누고 Unity 프로토타입의 36개 기능 중 31개를 코드와 에셋 수준에서 연결한 하루의 작업 기록."
date: 2026-02-24
categories: [Project]
tags: [Unity, AI]
---
...
계획표에서는 36개 기능 중 31개를 구현 상태로 옮겼다. 이 글에서 **86%는 기획 목록에 대응하는 코드와 기본 연결이 있다는 뜻**이다. 밸런스, 실제 기기 성능, 반복 플레이의 재미, 출시 품질이 86% 완성됐다는 뜻은 아니다. 아래 표도 제품 완성도가 아니라 그날의 구현 체크리스트다.
...
## 기능을 빠르게 연결할 수 있었던 조건

### GDD의 효과

GDD에 36개 기능의 입력, 수치, 완료 조건을 적어 두었기 때문에 매 요청에서 요구사항을 다시 설명하는 시간이 줄었다. 이것은 AI가 정확성을 보장했다는 뜻이 아니다. 생성한 기능은 컴파일, 테스트, Unity 실행 화면을 차례로 확인해야 했다. 기획 문서는 구현 속도를 높였고, 검증 절차는 잘못된 속도를 제어했다.
