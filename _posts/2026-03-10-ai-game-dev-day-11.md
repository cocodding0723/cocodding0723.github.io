...
리듬 액션에서 패링은 버튼을 눌렀다는 사실만으로 성공하지 않는다. 음악의 어느 박자에 눌렀는지 재고, 적의 공격 시점과 비교해야 한다. 11일차에는 화면 프레임이 흔들려도 같은 기준으로 입력 시간을 재는 리듬 엔진과, 그 결과를 네 등급으로 나누는 패링 판정을 만들었다.

여기서 `dspTime`은 Unity 오디오 시스템의 정밀한 시간값이고, **패링**은 적의 공격 타이밍에 맞춰 방어해 반격 기회를 만드는 동작이다. 아래 영상은 최종 게임이 아니라 이 두 기능을 확인하는 전투 프로토타입이다.
...
11일차에는 첫 전투 프로토타입이 실행됐고 EditMode 테스트 87개가 통과했다. 스크립트 44개라는 숫자보다 중요한 결과는 음악 시간 → 입력 등급 → 패링 상태 → 화면 피드백이 한 흐름으로 이어졌다는 점이다.

---

## 화면 프레임 대신 오디오 시간을 기준으로 삼기
