...
CodeGraph는 정확히 그 지점을 노린다. 코드를 미리 색인해 그래프로 만들어 두고, 에이전트는 grep 대신 그 그래프에 **질의**하게 만든다. 이 글은 내가 만든 게 아니라 남이 만든 프로젝트를 뜯어본 리뷰다. 영리한 부분은 영리하다고 적고, 의심스러운 부분은 의심스럽다고 적는다. 그리고 마지막에 "내 Claude Code에 이걸 적용할 것인가"를 솔직하게 답한다.
