...
BeatBox는 3D 큐브를 방향키로 굴려 다음 타일로 이동하는 리듬 게임이다. 정해진 박자와 방향을 모두 맞혀야 하고, 둘 중 하나라도 틀리면 게임이 끝난다.

이 글에서 **BPM**은 1분 동안의 박자 수, `dspTime`은 오디오 장치가 기준으로 삼는 정밀한 재생 시간이다. 화면 프레임 시간보다 오디오 시간을 기준으로 판정해야 음악과 입력의 오차를 일관되게 계산할 수 있다. DOTween은 크기와 색 같은 값을 시간에 따라 바꾸는 Unity용 애니메이션 도구다.

[AI로 게임 개발하기 시리즈](/blog/2026/02/20/ai-game-dev-day-1/)의 Rhythm Rogue와는 별개 프로젝트다. 같은 비트 엔진을 사용하되, 이 글에서는 큐브가 실제 모서리를 중심으로 구르는 방법과 입력 시각·방향을 함께 판정하는 흐름에 집중한다.

구현 순서는 큐브 이동 → 입력 판정 → 음악 시간 계산 → 타일 연출이다. 클래스 이름은 각 단계의 코드 위치를 찾기 위한 표지로만 사용한다.

---

## 큐브가 미끄러지지 않고 모서리로 구르게 하기
...
*BeatBox의 핵심은 스크립트 수가 아니라, 큐브의 물리적 회전과 오디오 기준 입력 판정을 분리한 뒤 같은 박자 이벤트로 연출을 묶은 것이다.*
