---
title: "AI로 게임 개발하기 9일차 — 첫 프로토타입을 마친 뒤 확인한 것"
description: "Unity 뱀서라이크 프로토타입의 기능 목록을 마무리한 뒤, 실제로 확인한 범위와 아직 검증하지 못한 부분, AI 보조 개발의 장단점을 돌아본다."
date: 2026-03-06
categories: [Project]
tags: [Unity, AI]
---
...
[1일차](/blog/2026/02/20/ai-game-dev-day-1/)에 "AI로 게임 개발 과정을 얼마나 줄일 수 있을까?"라는 질문으로 시작했다. 9일 뒤 기획 목록의 대부분을 실행할 수 있는 Vampire Survivors 스타일 프로토타입이 생겼다. 무기, 보스, 스테이지, 도감, 튜토리얼 화면은 연결됐지만 스토어에 출시한 완성품은 아니다.

오늘은 `PLAN.md`와 `TODO.md`를 대조해 무엇이 코드로 존재하고 무엇이 실제 실행으로 확인됐는지 점검했다. AdMob SDK도 연결했지만 실제 광고 운영과 수익 검증은 이후 과제다. 이 글의 목적은 기능 개수를 자랑하는 것이 아니라, 빠르게 만든 프로토타입에서 어떤 검증 빚이 남았는지 정리하는 데 있다.
