---
title: "요양시설 재활 플랫폼 개발 6주 — Claude Code 자동화 하네스를 실전에 투입한 기록"
description: "Karpathy의 vibe coding 개념과 ECC 오픈소스에서 영감을 받아 구축한 Claude Code 멀티 에이전트 하네스를 요양시설 재활 플랫폼 실프로젝트에 투입했다. 23개 slash command, 모델 라우팅 전략, GateGuard 훅이 에이전트를 실제로 막은 사례, PHI 암호화 호환성까지 6주 전체를 기록한다."
date: 2026-05-28
categories: [Project]
...
요양시설 입소자를 위한 **재활 운동 지원 플랫폼**이다. 시설 공용 태블릿에서 운영되는 Flutter 앱이 메인이고, Firebase Cloud Functions가 백엔드를 담당한다. 환자 데이터를 다루기 때문에 PHI(Protected Health Information) 암호화가 필수였고, QR 기반 인증으로 비밀번호 없이 공유 단말에서 개인 세션을 분리하는 구조를 잡았다.

개발 인원은 1~2명. Claude Code를 팀원으로 쓰는 구조였다.
...
Karpathy가 2025년 초 "vibe coding"이라는 개념을 트윗했다. 요지는 간단했다. "코드를 이해하려고 하지 말고 그냥 AI한테 시키고, 결과를 보고, 맞으면 넘어가라." 처음엔 장난처럼 들렸다. 실제로 1~2인 팀이 Claude Code로 작업하면서 이 개념이 현실이 됐다.
...
## Claude Code 하네스를 프로젝트에 맞게 확장하기
...
PHI 암호화 키 로테이션 자동화가 남아 있다. 현재는 키를 바꾸면 기존 암호화 데이터를 재암호화하는 작업을 수동으로 처리한다. Cloud Functions가 키 버전을 관리하고 구버전 키로 복호화 후 신버전으로 재암호화하는 마이그레이션 파이프라인을 만드는 게 목표다.

Patrol E2E 테스트 커버리지도 늘릴 예정이다. 현재는 happy path 위주인데 공유 태블릿 환경의 엣지 케이스(세션 중간에 다른 환자 QR 스캔, 네트워크 단절 중 운동 데이터 저장 등)를 시나리오로 추가해야 한다.

---

*에이전트에게 규칙을 설명하는 것만으로는 부족하다 — 규칙이 코드 실행 경로에 물리적으로 걸려 있어야 에이전트도, 사람도 지름길을 택하지 못한다.*
